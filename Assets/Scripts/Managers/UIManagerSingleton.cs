   ...
   public class UIManagerSingleton
   {
      ...
      private UIManagerSingleton()
      {
         ...
         foreach (RectTransform l_uiElement in Object.FindObjectsByType<RectTransform>(FindObjectsSortMode.None))
         {
            ...
            else if(l_uiElement.name == "Pause Menu")
            {
         ...
         m_redPotionIndicator.Init(false);

         l_travelledDistanceIndicator = Object.FindObjectsByType<TextMeshProUGUI>(FindObjectsSortMode.None).First(p_label => p_label.name == "Travelled Distance");

         foreach (ScalabbleButtonBehaviour l_button in Object.FindObjectsByType<ScalabbleButtonBehaviour>(FindObjectsSortMode.None))
         {
